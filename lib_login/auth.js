...
router.post('/login_process', function (request, response) {
    ...
    if (username && password) {             // id와 pw가 입력되었는지 확인
        ...
        db.query('SELECT * FROM login_user WHERE username = ? AND password = ?', [username, password], function(error, results, fields) {
...
router.get('/logout', function (request, response) {
    // const sessionId = request.sessionID; // 세션 ID 저장

    request.session.destroy(function (err) {
        if (err) {
            console.error(err);
        } else {
            // 세션 삭제가 완료된 후에 리다이렉트 수행
            // response.clearCookie(options.name); // 쿠키 삭제
            response.redirect('/');

            // 여기서 sessionId를 사용하여 세션 파일을 수동으로 삭제할 수도 있음
...
router.post('/register_process', function(request, response) {    
    ...
    if (username && password && password2) {
        ...
        db.query('SELECT * FROM login_user WHERE username = ?', [username], function(error, results, fields) { // DB에 같은 이름의 회원아이디가 있는지 확인
            if (error) throw error;
            if (results.length <= 0 && password == password2) {     // DB에 같은 이름의 회원아이디가 없고, 비밀번호가 올바르게 입력된 경우 
                db.query('INSERT INTO login_user (username, password) VALUES(?,?)', [username, password], function (error, data) {
